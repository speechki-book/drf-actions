...
def make_content_type():
    return ActionContentType.objects.create(content_type="author", table=Author._meta.db_table)
...
def test_insert_update_delete_events_via_trigger():
    ...
    events = EventJournal.objects.filter(content_type="author", reason="INSERT", object_id=str(author.id))
    assert events.count() == 1
    assert events.get().data == {"name": "Alice"}

    Author.objects.filter(pk=author.pk).update(name="Bob")
    update = EventJournal.objects.get(content_type="author", reason="UPDATE", object_id=str(author.id))
    assert update.data == {"name": "Bob"}

    author_id = author.id
    author.delete()
    # Model.delete() clears instance.pk, so the id must be captured beforehand.
    delete = EventJournal.objects.get(content_type="author", reason="DELETE", object_id=str(author_id))
...
def test_existing_rows_backfilled_on_content_type_creation():
    ...
    backfill = EventJournal.objects.get(content_type="author", reason="INSERT", object_id=str(author.id))
...
def test_trigger_dropped_on_content_type_delete():
    ...
    assert not EventJournal.objects.filter(content_type="author", object_id=str(author.id)).exists()
